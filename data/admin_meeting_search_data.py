...
time_sql = 'SELECT COUNT(1) FROM meeting WHERE order_start_time>="2020-12-02" and order_end_time<"2021-01-01" and deleted=0'

#根据会议类型-机构全公开查询
# type_meetings1 = MysqlDB().query('meeting', 'type=2 and deleted=0')
type_sql_1 = 'SELECT COUNT(1) FROM meeting WHERE type=2 and deleted=0'
#根据会议类型-平台全公开查询
# type_meetings2 = MysqlDB().query('meeting', 'type=0 and deleted=0')
type_sql_2 = 'SELECT COUNT(1) FROM meeting WHERE type=0 and deleted=0'
#根据会议类型-白名单查询
# type_meetings3 = MysqlDB().query('meeting', 'type=1 and deleted=0')
type_sql_3 = 'SELECT COUNT(1) FROM meeting WHERE type=1 and deleted=0'

# status:0未开始1已准备2，进行中8.已结束9.已取消
#根据会议状态-未开始查询
# status_meetings1 = MysqlDB().query('meeting', 'status=0 or status=1 and deleted=0')
status_sql_1 = 'SELECT COUNT(1) FROM meeting WHERE status=0 or status=1 and deleted=0'
#根据会议状态-会议中查询
# status_meetings2 = MysqlDB().query('meeting', 'status=2 and deleted=0')
status_sql_2 = 'SELECT COUNT(1) FROM meeting WHERE status=2 and deleted=0'
#根据会议状态-已结束查询
status_meetings3 = MysqlDB().query('meeting', 'status=8 and deleted=0')
status_sql_3 = 'SELECT COUNT(1) FROM meeting WHERE status=8 and deleted=0'

# 根据会议主题关键词查询查询
# keywords_meetings1 = MysqlDB().query('meeting', 'title like "%测试%" and deleted=0')
keywords_sql = 'SELECT COUNT(1) FROM meeting WHERE title like "%测试%" and deleted=0'
# 根据会议号关键词查询查询
...
meeting_search_data = [
    {'name': '根据所属机构查询', 'params': {'org': 'web自动化测试机构'}, 'case_status': True, 'expected_result': f'{org_meetings_sql}'},
