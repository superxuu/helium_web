...
# org_list = MysqlDB().sql_query(f"SELECT * FROM whitelist WHERE customer_id='{customerid}' and deleted=0")
org_sql = f"SELECT * FROM whitelist WHERE customer_id='{customerid}' and deleted=0"
# time_list = MysqlDB().sql_query("SELECT * FROM whitelist WHERE create_time>='2020-09-09' and create_time<'2020-10-30' and deleted=0")
time_sql = "SELECT * FROM whitelist WHERE create_time>='2020-09-09' and create_time<'2020-10-30' and deleted=0"
# keywords_list = MysqlDB().sql_query("SELECT * FROM whitelist WHERE name like '%测试%' and deleted=0")
keywords_sql = "SELECT * FROM whitelist WHERE name like '%测试%' and deleted=0"
...
all_sql = "SELECT * FROM whitelist WHERE customer_id='4861736231813292032' and create_time>='2020-09-09' and create_time<'2020-12-30' and name like '%科技%' and deleted=0"

whitelist_search_data = [
    {'name': '根据所属机构搜索白名单', 'params': {'org': '北鼎股份'}, 'case_status': True, 'expected_result': f'{org_sql}'},
    ...
    {'name': '根据全部条件搜索白名单', 'params': {'org': '北鼎股份','start_time': '2020-09-09', 'end_time':'2020-12-29','keywords': '科技'}, 'case_status': True, 'expected_result': f'{all_sql}'},
...
whitelist_add_data = [
    ...
    {'name': '名称为空创建白名单', 'params': {'org': '北鼎股份'}, 'expected_result': "Text('必填项',below=TextField('设置一个白名单名称')).exists()"},
