...
from data.add_meeting_data import add_meeting_data

@pytest.mark.run(order=3)
@allure.feature('会议管理')
class TestAddMeeting(object):

    @allure.story('预约会议')
    @pytest.mark.parametrize('data', add_meeting_data)
    def test_add_meeting(self, data, open_close_browser_with_admin_data):
        ...
        if data['params'].get('org', None):
            click(S('.el-input__inner', to_right_of=Text('归属机构')))
            write(data['params'].get('org'), into='请输入机构名称')
            click(ListItem(data['params'].get('org')))
        if type:
            click('平台全公开')
        if data['params'].get('title', None):
            write(data['params'].get('title'), into=TextField(to_right_of='会议主题'))
        ...
        if data['params'].get('AI', None):
            ...
            try:
                ...
                write(data['params'].get('title'), into='会议主题或会议室号')
                click('筛选')
