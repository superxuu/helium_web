...
@allure.feature('会议管理')
class TestOrgEditMeeting(object):

    @screenshoot
    @allure.story('会议编辑')
    @pytest.mark.parametrize('data', org_edit_meeting_data)
    def test_org_edit_meeting(self, data, open_close_browser_with_org_data):
        global title
        driver = open_close_browser_with_org_data
        allure.dynamic.description(data['name'])
        click(Button('清空'))
        if data['params'].get('status', None):
            click(TextField('全部', to_left_of='预约时间范围'))
